...
> Program ini dirancang untuk mensimulasikan sistem operasional **Optik Cahaya** berbasis Python 3.13. Sistem mengelola inventaris produk kacamata (frame, lensa, dan aksesori), pencatatan stok cabang, serta eksekusi transaksi penjualan lengkap dengan penambahan bonus otomatis untuk pembelian kacamata.
...
Sistem ini dibangun menggunakan tiga class utama yang saling berinteraksi secara modular:

### 1. `ProdukOptik`
Bertanggung jawab atas entitas produk (seperti Frame, Lensa, dan Aksesoris).
* **Atribut Class:**
  * `nama_optik`: Nama entitas toko (`'Optik Cahaya'`).
  * `produk_terdaftar`: Penghitung otomatis jumlah objek produk yang telah diinstansiasi.
  * `kategori_produk`: Daftar kategori barang yang valid (`['FRM', 'LNS', 'AKS']`).
* **Atribut Instance:**
...
* **Instance Method:**
  ...
  * `tampil_detail()`: Menampilkan rincian informasi produk (SKU, Nama, Kategori, Harga, Stok) dalam format tabel terstruktur.
* **Class Method (`buat_sku`):**
  * Merangkai kode SKU otomatis berbasis format `[KAT]-[MERK]-[FITUR]-[UKURAN]` sekaligus menginstansiasi objek `ProdukOptik` baru.
* **Static Method (`hitung_omset`):**
  * Menghitung potensi nilai omset dari suatu produk berdasarkan perkalian harga dan stok tanpa bergantung pada atribut instance.
...
* **Atribut Class:**
  * `nama_sistem`: Identitas sistem inventaris (`'Optik Inventory System'`).
  * `max_gudang`: Kapasitas maksimal penyimpanan gudang pusat (default: `1000`).
  * `lokasi_gudang`: Lokasi utama gudang pusat (`'Gudang Pusat Samarinda'`).
* **Atribut Instance:**
...
* **Instance Method:**
  ...
  * `tampil_stok()`: Menampilkan seluruh daftar produk beserta rincian detailnya pada cabang terkait.
...
* **Atribut Class:**
  * `nama_optik`: Nama entitas toko (`'Optik Cahaya'`).
  * `transaksi_selesai`: Penghitung akumulasi jumlah transaksi yang berhasil diproses.
...
* **Instance Method:**
  * `tambah_item_pembelian(produk, qty=1)`: Memotong stok barang secara langsung di inventaris, mengalkulasi subtotal, dan menambahkan ke daftar belanjaan.
  * `cetak_nota()`: Mencetak nota penjualan resmi, mengecek kelayakan bonus kacamata (`FRM`/`LNS`), serta mengupdate penghitung `transaksi_selesai`.
...
### 1. Inisialisasi Data & Penambahan Stok Cabang
* Sistem membuat 3 produk awal dengan metode berbeda (inisialisasi langsung dan melalui `buat_sku`).
* Produk didaftarkan ke cabang **Optik Cahaya Samarinda Central Plaza**.
* Menampilkan daftar stok awal cabang dalam bentuk tabel terformat di terminal.

### 2. Pengujian Method Class dan Static
* Memanggil `ProdukOptik.hitung_omset()` untuk memproyeksikan total nilai barang dari salah satu produk.
...
### 4. Pengujian Validasi Keamanan (Error Handling Setter)
